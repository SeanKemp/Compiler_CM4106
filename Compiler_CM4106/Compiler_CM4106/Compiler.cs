...
namespace Compiler
{
    ...
    public class Compiler
    {
        ...
        private void WriteFinalMessage()
        {
            ...
            {
                WriteLine("Errors were reported: ");
                foreach (System.Tuple<string, Position> error in Reporter.Errors)
                {
                    WriteLine($"Error: {error.Item1}, Position: {error.Item2.ToString()}");
                }
            }
            else
            {
                WriteLine("No Errors Reported");
            }
        }
