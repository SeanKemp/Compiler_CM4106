...
namespace Compiler.SemanticAnalysis
{
    ...
    public class DeclarationIdentifier
    {
        ...
        /// <summary>
        /// Carries out identification on a while forever command node
        /// </summary>
        /// <param name="whileForeverCommand">The node to perform identification on</param>
        private void PerformIdentificationOnWhileForeverCommand(WhileForeverCommandNode whileForeverCommand)
        {
            PerformIdentification(whileForeverCommand.Command);
        }
        ...
        {
            PerformIdentification(forCommand.FirstCommand);
            PerformIdentification(forCommand.Expression);
            PerformIdentification(forCommand.SecondCommand);
            PerformIdentification(forCommand.DoCommand);
        }
        ...
        {
            bool success = SymbolTable.Enter(constDeclaration.Identifier.IdentifierToken.Spelling, constDeclaration);
            if (!success) 
                Reporter.RecordError("Failed to enter a ConstDeclaration into symbol table", constDeclaration.Position);
        ...
        private void PerformIdentificationOnVarDeclaration(VarDeclarationNode varDeclaration)
        {
            ...
            bool success = SymbolTable.Enter(varDeclaration.Identifier.IdentifierToken.Spelling, varDeclaration);
            if (!success) 
                Reporter.RecordError("Failed to enter a VarDeclaration into symbol table", varDeclaration.Position);
        }
        ...
        /// <summary>
        /// Carries out identification on an call expression node
        /// </summary>
        ...
        private void PerformIdentificationOnOperator(OperatorNode operation)
        {
            ...
            if (declaration == null)
                Reporter.RecordError($"Couldn't retrieve operator {operation.OperatorToken.Spelling} from symbol table", operation.Position);
        ...
        private void PerformIdentificationOnIdentifier(IdentifierNode identifier)
        {
            ...
            if (declaration == null) 
                Reporter.RecordError($"Couldn't retrieve identifier {identifier.IdentifierToken.Spelling} from symbol table", identifier.Position);
