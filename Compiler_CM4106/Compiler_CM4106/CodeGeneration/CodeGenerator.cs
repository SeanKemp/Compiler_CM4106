    ...
    public class CodeGenerator
    {
        ...
        private void GenerateCodeForAssignCommand(AssignCommandNode assignCommand)
        {
            ...
            if (assignCommand.Identifier.Declaration is IVariableDeclarationNode varDeclaration)
            {
                ...
                Debugger.Write("The identifier is not a variable and you should have picked this " +
                    "problem up during type checking");
        }
        ...
        private void GenerateCodeForWhileCommand(WhileCommandNode whileCommand)
        {
            ...
            code.AddInstruction(OpCode.JUMPIF, Register.CB, TrueValue, commandJumpAddress);
        }

        /// <summary>
        /// Generates code for a while forever command node
        /// </summary>
        /// <param name="whileForeverCommand">The node to generate code for</param>
        private void GenerateCodeForWhileForeverCommand(WhileForeverCommandNode whileForeverCommand)
        {
            Debugger.Write("Generating code for While Forever Command");
            Address commandJumpAddress = code.NextAddress;
            GenerateCodeFor(whileForeverCommand.Command);
            code.AddInstruction(OpCode.JUMP, Register.CB, 0, commandJumpAddress);
        }

        /// <summary>
        /// Generates code for a for command node
        /// </summary>
        /// <param name="forCommand">The node to generate code for</param>
        private void GenerateCodeForForCommand(ForCommandNode forCommand)
        {
            Debugger.Write("Generating code for For Command");
            GenerateCodeFor(forCommand.FirstCommand);
            Address evalJumpAddress = code.NextAddress;
            GenerateCodeFor(forCommand.Expression);
            Address forFalseJumpAddress = code.NextAddress;
            code.AddInstruction(OpCode.JUMPIF, Register.CB, FalseValue, 0);
            GenerateCodeFor(forCommand.DoCommand);
            GenerateCodeFor(forCommand.SecondCommand);
            code.AddInstruction(OpCode.JUMP, Register.CB, 0, evalJumpAddress);
            code.PatchInstructionToJumpHere(forFalseJumpAddress);
        }
